...
class UrlCollection(Collection):
    ...
    def collect(self,add_actor_websites=False,with_update_ref=False):
        ...
        if add_actor_websites:
            url_iter = self._add_actor_websites(url_iter)
        ...
        rev_url_data = {value["unpacked"]:key for key, value in self.project["cleaned_urls"].data.items() if value is not None}
        for org_url,sources in url_iter:
            print (org_url)
            at_least_one_collected = False
            if org_url in self.project["cleaned_urls"].data and self.project["cleaned_urls"].data[org_url] is not None:
                cleaned_url = self.project["cleaned_urls"].data[org_url]["unpacked"]
            else:
                clean_data = lc.clean_url(org_url,with_unpack=False)
                self.project["cleaned_urls"].simple_update({org_url:clean_data})
            ...
            if cleaned_url is not None:
                ...
                for source in sources:
                    if not self.only_domains and source in self.project["referal_data"].data[org_url]["data"]:
                        continue
                    if not self.only_domains and cleaned_url in rev_url_data and source in self.project["referal_data"].data[rev_url_data[cleaned_url]]["data"]:
                        continue
                    ...
                    try:
                        if self.only_domains:
                            if with_update_ref:
                                ...
                                continue
                                #pass
                            referal_data = method.domain_referals(cleaned_url,start_date=self.start_date,end_date=self.end_date)
                            ...
                            referal_data = method.url_referals(cleaned_url,start_date=self.start_date,end_date=self.end_date)
